  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      if (filter) {
        ...
        const allObjectFilter = await Doctor.find({ [label]: { '$regex': filter[1] } }).limit(limit).skip(page * limit).sort({ createdAt: -1, updatedAt: -1 })
        resolve({
          status: 'OK',
          message: 'Success',
      ...
      if (sort) {
        ...
        const allDoctorSort = await Doctor.find().limit(limit).skip(page * limit).sort(objectSort).sort({ createdAt: -1, updatedAt: -1 })
        resolve({
          status: 'OK',
          message: 'Success',
          data: allDoctorSort,
      ...
      if (!limit) {
        ...
        allDoctor = await Doctor.find().limit(limit).skip(page * limit).sort({ createdAt: -1, updatedAt: -1 })
      }
      resolve({
        status: 'OK',
        message: 'Success',
