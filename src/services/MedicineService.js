...
const createMedicine = (newMedicine) => {
  return new Promise(async (resolve, reject) => {
    const { name, image, type, countInStock, price, description, selled } =
      newMedicine;
    try {
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      if (filter) {
        ...
        const allObjectFilter = await Medicine.find({
      ...
      if (sort) {
        ...
        const allMedicineSort = await Medicine.find()
          .limit(limit)
          .skip(page * limit)
          .sort(objectSort)
          .sort({ createdAt: -1, updatedAt: -1 });
      ...
      if (!limit) {
        allMedicine = await Medicine.find().sort({
          createdAt: -1,
          updatedAt: -1,
        });
      } else {
        allMedicine = await Medicine.find()
          .limit(limit)
          .skip(page * limit)
          .sort({ createdAt: -1, updatedAt: -1 });
      }
